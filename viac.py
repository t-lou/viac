...
def load_list(code: str) -> list:
    ...
    print(f"opening {url}")
    text = (
        open(DEBUG_FILENAME, "r", encoding="utf-8").read()
        if DEBUG_READ
        else opener.open(url).read().decode("utf8")
    )
    ...
    for text_link, text_desc in zip(links, descs):
        item = {
            key: re.findall(pattern, text_link) for key, pattern in patterns.items()
        }
        item["title"], _ = split_text(text_desc, "</span>", "</div>")

        for key_link in ("link", "pdf"):
            item[key_link] = (
                f'=HYPERLINK("{root}{item[key_link][0]}", "goto")'
                if bool(item[key_link])
                else ""
            )
...
def export():
    ...
    with pandas.ExcelWriter(get_name(), engine="xlsxwriter") as writer:
        for config in configs:
            ...
            is_interesting = [
                any(
                    kw_config in kw_title
                    for kw_config in config["keywords"]
                    for kw_title in item["keywords"]
                )
                for item in summary
            ]

            to_write = {
                key: [item[key] for item in summary] for key in ("title", "link", "pdf")
            }

            pandas.DataFrame(to_write).style.apply(
                lambda x: highlight(x, mask=is_interesting), axis=None
            ).to_excel(writer, sheet_name=config["name"], index=False)
